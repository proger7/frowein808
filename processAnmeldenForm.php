...
switch ($langLink) {
    case 'de':
        $mls = '<h2 style="color:#e2001a">Diese Email Adresse ist bereits im System hinterlegt. Bitte registrieren Sie sich mit einer anderen Adresse.</h2>';
        $captchaErrMessage = "<h2>Sie haben einen ungültigen Captcha-Wert eingegeben</h2>";
        $successMessage = "<h2>Danke, Ihre Registrierung war erfolgreich! Bitte prüfen Sie Ihr E-Mail-Konto für die Bestätigung.</h2>
                        <p>Bitte prüfen Sie auch Ihren Spam- und Junk E-Mail-Ordner.<br />
                        Tipp: Speichern Sie unsere E-Mailadresse (<a href='mailto:newsletter@frowein808.de'>newsletter@frowein808.de</a>) in Ihren Kontakten, damit E-Mails von uns sicher in Ihrem Postfach landen und nicht als Spam markiert werden.</p>";
    ...
    break;
    case 'en':
        $mls = '<h2 style="color:#e2001a">This email address is already stored in the system. Please register with a different address.</h2>';
        $captchaErrMessage = "<h2>You entered an invalid captcha value</h2>";
        $successMessage = "<h2>Thank you, your registration was successful! Please check your email account for confirmation.</h2>
                         <p>Please also check your spam and junk email folders.<br />
                         Tip: Save our e-mail address (<a href='mailto:newsletter@frowein808.de'>newsletter@frowein808.de</a>) in your contacts so that e-mails from us end up safely in your mailbox and not be marked as spam.</p>";
...
// Anmeldung function
$messageNewsletterForm = "";



/* Honeypot */
if( !empty($_POST['secNewsletter']) ) {
    die();
}

if ($_POST['honeypotNewsletter'] == 1) {
    die();
}

if( $_SESSION['capcha'] != $_POST['kapcha'] ) {
    $messageNewsletterForm .= $captchaErrMessage;
}
/* Honeypot END */

if( $_SESSION["token_anmelden"] != $_POST["token_anmelden"] || !isset($_POST['token_anmelden']) || !isset($_SESSION['token_anmelden']) ) {
    exit("INVALID TOKEN");
}


if(isset($_POST['gender-newsletter']) ) {
    	

    	$check_user = $DB->select("SELECT * FROM newsletter_users WHERE kundennummer = 0 AND user_email != '' AND `if_unsubscribe` = 0 AND user_email='".$_POST['newsletter-pemail']."'");

    	// echo '<pre>'; var_dump($check_user); echo '</pre>';
        
    	if( !empty($check_user) &&  isset($_POST['newsletter-vorname']) && isset($_POST['newsletter-name']) && isset($_POST['newsletter-pemail']) ) {
    		$messageNewsletterForm .= $mls;
    	} elseif( empty($check_user) && ( empty($_POST['newsletter-vorname']) || empty($_POST['newsletter-name']) || empty($_POST['newsletter-pemail']) )  ) {

    		$messageNewsletterForm .= $errorMessage;

    	} else {

            if( !empty($_POST['newsletter-vorname']) && !empty($_POST['newsletter-name']) && !empty($_POST['newsletter-pemail']) && !empty($_POST['kapcha']) && ( $_SESSION['capcha'] == $_POST['kapcha'] ) && isset($_POST["token_anmelden"]) && isset($_SESSION["token_anmelden"]) && isset($_SESSION["token-expire"]) && $_SESSION["token_anmelden"]==$_POST["token_anmelden"] ) {
                ...
                $user_titel = "";
                $id = $DB->lastindex_query("INSERT INTO `newsletter_users_temp` ( `update`,  `language_id`, `status`, `order_id`, `user_email`, `user_anrede`, `user_titel`,   `user_nachname`, `user_name` , `user_createdate`, `user_mail_format`,   `user_group`   ) VALUES 
                (   now(),   ".$language_id.",  0,  1,  '".$_POST['newsletter-pemail']."',  '".$user_anrede."',  '".$user_titel."',     '".$_POST['newsletter-name']."',    '".$_POST['newsletter-vorname']."',     now(),  0,   55 );");
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" class="body">
      <tr>
          ...
          <div class="content">
            ...
            <table role="presentation" class="main">
                    ...
                    <tr>
                      <td>
                        ...
                        <div style="text-align: center;margin: 65px 0">
                          <a class="abmeldung_btn" href="'.$URL_ROOT.'activate/'.$id.'">Anmeldung bestätigen</a>
                        </div>
              ...
              <tr id="prefooter">
                ...
                <td class="wrapper">
                  <a href="tel:+49 7432 956-0" style="display: flex !important;align-items: center !important;width:155px !important" class="tel-link"><img style="width: auto;height: 100%;padding-top: 3px" src="'.$URL_ROOT.'emailTemplate1/tel.png">+49 7432 956-0</a>
                </td>
                <td class="wrapper">
        ...
        ';
                ...
                $subject ='=?UTF-8?B?'.base64_encode("Frowein Newsletter - Anmeldung bestätigen"). "?=";

                $mailfrom = "info@frowein808.de";
                $recipient = ($_POST['newsletter-pemail']);
                ...
                unset($_SESSION["token_anmelden"]);
                unset($_SESSION["token-expire"]);

                if (!filter_var($_POST['newsletter-pemail'], FILTER_VALIDATE_EMAIL)) {
                    $messageNewsletterForm .= $errorEmail;
                } else {
                    $messageNewsletterForm .= $successMessage;
                }
    	...
    	echo $messageNewsletterForm;

        
    } else {
    	$user_groups = $DB->select("SELECT * FROM newsletter_interestgroup WHERE status=1");
    	if($_POST['anrede'] == "Frau"){
    		$frau_sel = "selected";
    		$herr_sel = "";
    	} else if($_POST['anrede'] = "Herr"){
    		$frau_sel = "";
    		$herr_sel = "selected";
    	}
    }
